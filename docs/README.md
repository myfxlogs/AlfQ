...
>
> 🤖 **AI Agent 首次进入仓库**：阅读路径为
> 1. `/AGENT.md`（项目身份/红线）
> 2. `/docs/tasks/AGENT-RUNBOOK.md`（**唯一执行入口**：阅读顺序、执行循环、提交规范）
> 3. `/docs/tasks/MASTER-ROADMAP.md`（当前任务清单）
> 4. 按 RUNBOOK §1 "额外按需阅读" 表，触发式读其它 doc。
>
> **不要**预读 01~28 全部文档；上下文会爆。
...
| 29 | [29-MT4-MT5-差异参考.md](./29-MT4-MT5-差异参考.md) | **MT4 vs MT5 网关能力差异**：symbol / 行情 / 历史 / 订单 RPC 字段对照、嵌套差异、坑列表 — 凡涉 MT 平台必读 |
